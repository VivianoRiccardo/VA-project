const ActionType = {
  ...
  const isObject = (data) => {
    return typeof data === "object" && data != null && Object.getPrototypeOf(data) === Object.prototype && !Array.isArray(data) && !isMap(data) && !isSet(data);
  };
  ...
  const readableTick = (num) => {
    ...
    readable = readable.replace(/(e)\+(\d+)/, "$1$2");
    readable = readable.replace(/0+(e-?\d+)$/, "$1");
    readable = readable.replace(/(\.[0-9]+?)0+$/, "$1");
    readable = readable.replace(/\.(e)/, "$1");
    return readable;
  };
  ...
  const internalToFilter = (filter) => {
    return comparePrimitive(filter.value0, filter.value1) === 1 ? [filter.value1, filter.value0] : [filter.value0, filter.value1];
  };
  const internalToFilters = (filters) => {
    return Object.keys(filters).reduce((acc, key) => {
      acc[key] = filters[key].map((filter) => internalToFilter(filter)).sort((a, b) => comparePrimitive(a[0], b[0]));
      return acc;
    }, {});
  };
  ...
  const dimensionRanges = {
    ...
    "layer-free-decay": [1e-3, 0.1],
    "layer-split-factor": [1, 16],
    "learning-rate": [1e-4, 0.1],
    "learning-rate-decay": [1e-6, 1e-3],
    loss: [1.7, 2.4],
      ...
      acc[dimension.key] = new Array(count).fill(null).map((_, index) => {
        if (dimension.type === DimensionType.Categorical) {
          if (dimension.categories) {
            return random ? randomItem(dimension.categories) : idempotentItem(dimension.categories, index);
          }
        } else if (dimension.type === DimensionType.Linear) {
          const range = dimensionRanges[dimension.key];
          if (range) {
            return random ? randomNumber(range[1], range[0]) : idempotentNumber(range[1], range[0], count, index);
          }
        } else if (dimension.type === DimensionType.Logarithmic) {
          const range = dimensionRanges[dimension.key];
          if (range && dimension.logBase) {
            return random ? randomLogNumber(dimension.logBase, range[1], range[0]) : idempotentLogNumber(dimension.logBase, range[1], range[0], count, index);
          }
        }
  ...
  const generateDimensions = (dimCount = DEFAULT_DIMENSION_COUNT, random = true) => {
    const dims = new Array(dimCount - 1).fill(null).map((_, index) => {
      return random ? randomItem(dimensionSamples) : idempotentItem(dimensionSamples, index);
    });
    const metricDimension = random ? randomItem(metricDimensionSamples) : idempotentItem(metricDimensionSamples, 0);
  ...
  var tester = /* @__PURE__ */ Object.freeze({
    ...
    dimensionRanges,
    dimensionSamples,
    metricDimensionSamples,
    generateData,
    generateDimensions
  });
  class Hermes {
    constructor(target, dimensions, config, data, callback, callback2) {
      ...
      if (data){
        ...
        this.setKeysData(data)
        this.setKeysFalse(data)
      }
      this.listeners = {
        dblclick: this.handleDoubleClick.bind(this),
        mousedown: this.handleMouseDown.bind(this),
        mousemove: this.config.interactions.throttleDelayMouseMove === 0 ? this.handleMouseMove.bind(this) : throttle((e) => this.handleMouseMove.bind(this)(e), this.config.interactions.throttleDelayMouseMove),
    ...
    setConfig(config = {}, redraw = true) {
      ...
      this.resizeObserver = new ResizeObserver(this.config.interactions.throttleDelayResize === 0 ? this.handleResize.bind(this) : throttle((entries) => this.handleResize.bind(this)(entries), this.config.interactions.throttleDelayResize));
    ...
    setKeysData(data) {
      for (let key in data) {
        ...
        for(let i = 0; i < data[key].length; i++){
          if (!(data[key][i] in this.listValues[key]))
            this.listValues[key][data[key][i]] = {'identifiers': [data['names'][i]], 'selected':false}
          else{
            this.listValues[key][data[key][i]]['identifiers'].push(data['names'][i])
          }
        }
      }
    }
    ...
    setDimensions(dimensions, redraw = true) {
      ...
      const direction = this.config.direction === Direction.Horizontal ? Direction.Vertical : Direction.Horizontal;
      ...
      this.dimensions = clone(dimensions).map((dimension) => {
        ...
        if (dimension.type === DimensionType.Linear || dimension.type === DimensionType.Logarithmic) {
    ...
    redraw() {
      ...
      if(this.somethingChanged){
        this.customcallback(this.listValues, this.keysAreFalse)
        this.somethingChanged = false;
    ...
    calculateLayout() {
      ...
      const _ = {
        dims: {
          list: new Array(this.dimensions.length).fill(void 0).map(() => ({axes: {}, label: {}, layout: {}})),
      ...
      _dsl.cos = isLabelAngled ? Math.cos((_a = dimLabelStyle.angle) !== null && _a !== void 0 ? _a : 0) : void 0;
      _dsl.sin = isLabelAngled ? Math.sin((_b = dimLabelStyle.angle) !== null && _b !== void 0 ? _b : 0) : void 0;
      _dsl.rad = dimLabelStyle.angle || (isHorizontal ? void 0 : isLabelBefore ? -Math.PI : 0);
      _dsl.maxLengthCos = 0;
      _dsl.maxLengthSin = 0;
      ...
      this.dimensions.forEach((dimension, i) => {
        ...
        if (isLabelAngled) {
          _dlily.spaceBefore = isHorizontal ? _dlil.lengthCos < 0 ? -_dlil.lengthCos : 0 : _dlil.lengthSin > 0 ? _dlil.lengthSin : 0;
          _dlily.spaceAfter = isHorizontal ? _dlil.lengthCos > 0 ? _dlil.lengthCos : 0 : _dlil.lengthSin < 0 ? -_dlil.lengthSin : 0;
        } else {
      ...
      for (let i = 0; i < _.dims.list.length; i++) {
        ...
        if (isHorizontal) {
          ...
          _dlily.labelPoint = {
            ...
            y: isLabelBefore ? _dsa.start - dimLabelStyle.offset - _l.padding[0] : _dsa.stop + dimLabelStyle.offset - _l.padding[0]
          };
        } else {
          if (dimLayout === DimensionLayout.AxisEvenlySpaced) {
          ...
          _dlily.labelPoint = {
            x: isLabelBefore ? _dsa.start - dimLabelStyle.offset - _l.padding[1] : _dsa.stop + dimLabelStyle.offset - _l.padding[1],
    ...
    calculateStyles() {
      ...
      for (let i = 0; i < _dl.length; i++) {
        ...
        const isAxisActive = (_ixsa.type === ActionType.FilterCreate || _ixsa.type === ActionType.FilterMove || _ixsa.type === ActionType.FilterResizeAfter || _ixsa.type === ActionType.FilterResizeBefore) && _ixsa.dimIndex === i;
        const isAxisFocused = ((_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.DimensionAxis || (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.Filter || (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.FilterResize) && (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.dimIndex) === i;
        _s[i] = _s[i] || {};
        _s[i].label = {
          ..._osd.label,
        ...
        _s[i].filters = filters.map((filter, j) => {
          const isFilterFocused = ((_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.Filter || (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.FilterResize) && (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.dimIndex) === i && (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.filterIndex) === j;
          const isFilterAxisHover = isAxisActive || isAxisFocused && !isFilterFocused;
          const isFilterHover = isFilterFocused && _ixsa.filterIndex === void 0;
    ...
    getFocusByPoint(point) {
      ...
      for (let i = 0; i < this._.dims.list.length; i++) {
        ...
        if ((isPointInTriangle(point, labelBoundary[0], labelBoundary[1], labelBoundary[2]) || isPointInTriangle(point, labelBoundary[2], labelBoundary[3], labelBoundary[0])) && !this.dimensions[i].disableDrag) {
          return {dimIndex: i, type: FocusType.DimensionLabel};
        }
        const axisBoundary = layout.axisBoundary;
        if (isPointInTriangle(point, axisBoundary[0], axisBoundary[1], axisBoundary[2]) || isPointInTriangle(point, axisBoundary[2], axisBoundary[3], axisBoundary[0])) {
          const filters = this.filters[key] || [];
    ...
    updateActiveLabel() {
      ...
      for (let i = 0; i < _dl.length; i++) {
        ...
        if (_ixsa.dimIndex !== i && !this.dimensions[i].disableDrag && Math.abs(axisDistance) < DIMENSION_SWAP_THRESHOLD) {
    ...
    setActiveFilter(key, pos, value) {
      ...
      if (index !== -1) {
        ...
        if (pos >= _ixf.active.p0 && pos <= _ixf.active.p0 + FILTER_RESIZE_THRESHOLD / _dsa.length) {
          _ixsa.type = ActionType.FilterResizeBefore;
        } else if (pos >= _ixf.active.p1 - FILTER_RESIZE_THRESHOLD / _dsa.length && pos <= _ixf.active.p1) {
    ...
    updateActiveFilter(e) {
      ...
      const isFilterAction = _ixsa.type === ActionType.FilterCreate || _ixsa.type === ActionType.FilterMove || _ixsa.type === ActionType.FilterResizeAfter || _ixsa.type === ActionType.FilterResizeBefore;
      ...
      if (_ixsa.type === ActionType.FilterMove) {
        ...
        } else if (_ixf.active.p1 > 1) {
          _ixf.active.p0 = 1 - startLength;
          _ixf.active.p1 = 1;
        }
        ...
        _ixf.active.p0 = capDataRange(_ixf.active.p0, [0, 1]);
        _ixf.active.value0 = this.dimensions[index].scale.percentToValue(_ixf.active.p0);
      } else {
        _ixf.active.p1 = (_ixsa.p1[filterKey] - bound[filterKey] - axisStart) / _dsa.length;
        _ixf.active.p1 = capDataRange(_ixf.active.p1, [0, 1]);
        _ixf.active.value1 = this.dimensions[index].scale.percentToValue(_ixf.active.p1);
      }
      if (e.type !== "mouseup")
        return;
    ...
    updateCursor() {
      ...
      if (_ixsa.type !== ActionType.None) {
        if (_ixsa.type === ActionType.FilterMove || _ixsa.type === ActionType.LabelMove) {
          cursor = "grabbing";
        } else if (_ixsa.type === ActionType.FilterResizeAfter || _ixsa.type === ActionType.FilterResizeBefore) {
          cursor = isHorizontal ? "ns-resize" : "ew-resize";
        } else if (_ixsa.type === ActionType.FilterCreate) {
          cursor = "crosshair";
        }
      } else if (_ixsf !== void 0) {
    ...
    setListKeysAllToBoolean(flag, key){
        for(let key2 in this.listValues[key]){
          this.listValues[key][key2]['selected'] = flag;
        }
    }
    ...
    draw() {
      ...
      for (let k = 0; k < this.dataCount; k++) {
        ...
        const series = this.dimensions.map((dimension, i) => {
          ...
          let temp_l = this.getDataFilters(this.updated);
          if(temp_l.length > 1 || temp_l[0]){
            this.updated = !this.updated
            this.temp_l = temp_l
          }

          if(this.temp_l.includes(k)){
            hasFilters2 = true;
          }
          
          const pos = (_b = (_a2 = dimension.scale) === null || _a2 === void 0 ? void 0 : _a2.valueToPos(value)) !== null && _b !== void 0 ? _b : 0;
          ...
          if (dimColorKey === key) {
            const percent2 = (_f = (_e = dimension.scale) === null || _e === void 0 ? void 0 : _e.valueToPercent(value)) !== null && _f !== void 0 ? _f : 0;
            ...
            const scaleColor = scale2rgba(reverse ? colors.slice().reverse() : colors, percent2);
          ...
          if (_filters[key] && _filters[key].length !== 0) {
            ...
            if (!hasMatchedFilter){
              ...
              this.keysAreFalse[key] = false;
            }

            if(this.listValues[key][value]['selected'] != hasMatchedFilter){
              this.listValues[key][value]['selected'] = hasMatchedFilter;
              this.somethingChanged = true;
            }
            
          }

          if(!hasFilters && !this.keysAreFalse[key]){
            this.setListKeysAllToBoolean(false,key);
            this.keysAreFalse[key] = true;
            this.somethingChanged = true;
          }
        ...
        if ((hasFilters && !isFilteredOut) || hasFilters2){
          dataDefaultStyle = dataStyle.custom;
          lthis.push({'series':JSON.parse(JSON.stringify(series)),'isHorizontal':JSON.parse(JSON.stringify(isHorizontal)), 'style':JSON.parse(JSON.stringify(dataStyle.path)),'dataDefaultStyle':JSON.parse(JSON.stringify(dataDefaultStyle))})
        }

        drawData(this.ctx, series, isHorizontal, dataStyle.path, dataDefaultStyle);
      }
      for(let t = 0; t < lthis.length; t++){
        drawData(this.ctx, lthis[t]['series'], lthis[t]['isHorizontal'],lthis[t]['style'],lthis[t]['dataDefaultStyle'] );
      }

      const labelAdjust = dimStyle.label.angle == null && isHorizontal;
      const dimTextStyle = {
        textAlign: labelAdjust ? "center" : void 0,
        textBaseline: labelAdjust ? isLabelBefore ? "bottom" : "top" : void 0
      };
      this.dimensions.forEach((dimension, i) => {
        var _a2;
        ...
        drawText(this.ctx, dimension.labelTruncated, x, y, (_a2 = _dsl.rad) !== null && _a2 !== void 0 ? _a2 : 0, style);
      });
      const tickAdjust = axesStyle.label.angle == null && isHorizontal;
      const tickTextStyle = {
        textAlign: tickAdjust ? void 0 : "center",
        textBaseline: tickAdjust ? void 0 : isAxesBefore ? "bottom" : "top"
      };
      _dl.forEach((dim, i) => {
        const key = this.dimensions[i].key;
        ...
        for (let j = 0; j < tickLabels.length; j++) {
          ...
          if (tickLabel[0] === "*") {
            if ((_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.dimIndex) === i && ((_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.DimensionAxis || (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.Filter || (_ixsf === null || _ixsf === void 0 ? void 0 : _ixsf.type) === FocusType.FilterResize)) {
          ...
          const rad = axesStyle.label.angle != null ? axesStyle.label.angle : isHorizontal && isAxesBefore ? Math.PI : 0;
  ...
  export {ActionType, DimensionLayout, DimensionType, Direction, FocusType, LabelPlacement, PathType};